...
export const CHAIN_EVENT_TEMPLATES: EventTemplate[] = [
  {
    ...
    type: 'crash',
    impact: { driftModifier: -0.12, volatilityModifier: 0.3, severity: 'high' },
    duration: 80,
    weight: 2,
  },
  {
  ...
  },
  {
    ...
    type: 'crash',
    impact: { driftModifier: -0.18, volatilityModifier: 0.4, severity: 'critical' },
    duration: 120,
    weight: 1,
  },
  {
    ...
    type: 'crash',
    impact: { driftModifier: -0.12, volatilityModifier: 0.25, severity: 'high' },
    duration: 90,
    weight: 2,
  },
  {
...
export const HISTORICAL_EVENTS: HistoricalEvent[] = [
  ...
  // ═══ 1997 ═══
  // v7.4: IMF 위기 drift 50% 감소 (게임 밸런스 — 원래 -0.12~-0.25)
  { year: 1997, month: 1, title: '한보그룹 부도', description: '한보철강이 부도처리되며 금융 불안이 시작됩니다.', type: 'crash', impact: { driftModifier: -0.06, volatilityModifier: 0.2, severity: 'high' }, duration: 100, weight: 2, affectedSectors: ['finance', 'industrial'], chainEvents: [{ templateIndex: 3, probability: 0.6, delayTicks: [50, 100] }] },
  { year: 1997, month: 3, title: '삼미·진로 등 대기업 연쇄 부도', description: '대기업 연쇄 부도로 금융권 부실이 심화됩니다.', type: 'crash', impact: { driftModifier: -0.08, volatilityModifier: 0.25, severity: 'critical' }, duration: 120, weight: 1, affectedSectors: ['finance'] },
  { year: 1997, month: 7, title: '태국 바트화 폭락, 아시아 위기 시작', description: '태국 바트화 폭락으로 아시아 금융위기가 시작됩니다.', type: 'global', impact: { driftModifier: -0.09, volatilityModifier: 0.35, severity: 'critical' }, duration: 200, weight: 1, chainEvents: [{ templateIndex: 4, probability: 0.8, delayTicks: [30, 80] }, { templateIndex: 13, probability: 0.7, delayTicks: [20, 60] }] },
  { year: 1997, month: 10, title: '외환보유고 급감', description: '원화 방어에 실패하며 외환보유고가 위험 수준으로 감소합니다.', type: 'global', impact: { driftModifier: -0.08, volatilityModifier: 0.3, severity: 'critical' }, duration: 80, weight: 1 },
  { year: 1997, month: 11, title: 'IMF 구제금융 신청', description: '한국이 IMF에 구제금융을 신청합니다. 국가 신용도 폭락.', type: 'crash', impact: { driftModifier: -0.12, volatilityModifier: 0.4, severity: 'critical' }, duration: 250, weight: 1, chainEvents: [{ templateIndex: 0, probability: 0.9, delayTicks: [10, 30] }, { templateIndex: 8, probability: 0.8, delayTicks: [50, 100] }, { templateIndex: 9, probability: 0.6, delayTicks: [100, 200] }], lessonText: '교훈: 국가도 파산할 수 있습니다. 포트폴리오의 20% 이상은 현금으로 비축하고, 과도한 레버리지를 경계하세요.' },
  { year: 1997, month: 12, title: '환율 2000원 돌파', description: '원/달러 환율이 2000원을 넘어서며 공포가 확산됩니다.', type: 'crash', impact: { driftModifier: -0.10, volatilityModifier: 0.35, severity: 'critical' }, duration: 80, weight: 1 },

  // ═══ 1998 ═══
  { year: 1998, month: 1, title: '금 모으기 운동', description: '국민적 금 모으기 운동으로 외환 위기 극복 의지를 다집니다.', type: 'global', impact: { driftModifier: 0.06, volatilityModifier: -0.1, severity: 'medium' }, duration: 120, weight: 3 }, // v7.4: 반등 강화 0.02→0.06
  ...
  { year: 1998, month: 5, title: '러시아 모라토리엄', description: '러시아가 디폴트를 선언하며 글로벌 불안이 재확산됩니다.', type: 'global', impact: { driftModifier: -0.12, volatilityModifier: 0.3, severity: 'high' }, duration: 100, weight: 2 },
  { year: 1998, month: 8, title: 'LTCM 파산 위기', description: '롱텀캐피탈 매니지먼트 파산 위기로 글로벌 금융 시스템 불안.', type: 'crash', impact: { driftModifier: -0.15, volatilityModifier: 0.35, severity: 'critical' }, duration: 80, weight: 1, affectedSectors: ['finance'] },
  { year: 1998, month: 10, title: '외환위기 회복 조짐', description: '환율이 안정되기 시작하며 회복 기대감이 형성됩니다.', type: 'boom', impact: { driftModifier: 0.06, volatilityModifier: 0.15, severity: 'medium' }, duration: 120, weight: 3, chainEvents: [{ templateIndex: 1, probability: 0.6, delayTicks: [40, 80] }], lessonText: '교훈: 가장 어두운 밤이 지나면 새벽이 옵니다. 위기 속에서 매수한 투자자가 가장 큰 수익을 얻습니다.' },
  ...
  { year: 2000, month: 3, title: '나스닥 폭락, 닷컴 버블 붕괴', description: '나스닥이 폭락하며 전세계 IT 버블이 꺼집니다.', type: 'crash', impact: { driftModifier: -0.20, volatilityModifier: 0.5, severity: 'critical' }, duration: 250, weight: 1, affectedSectors: ['tech', 'telecom'], chainEvents: [{ templateIndex: 0, probability: 0.8, delayTicks: [30, 60] }, { templateIndex: 18, probability: 0.7, delayTicks: [80, 150] }], lessonText: '교훈: PER 100배를 넘는 주식은 결국 제자리로 돌아옵니다. 스토리가 아닌 실적을 보고 투자하세요.' },
  { year: 2000, month: 5, title: '코스닥 대폭락', description: '코스닥 지수가 반토막 나며 개인투자자 큰 손실.', type: 'crash', impact: { driftModifier: -0.18, volatilityModifier: 0.45, severity: 'critical' }, duration: 180, weight: 1, affectedSectors: ['tech'] },
  ...
  { year: 2001, month: 9, title: '9/11 테러 공격', description: '미국 동시다발 테러로 전세계 금융시장이 패닉에 빠집니다.', type: 'crash', impact: { driftModifier: -0.22, volatilityModifier: 0.6, severity: 'critical' }, duration: 100, weight: 1, chainEvents: [{ templateIndex: 0, probability: 0.9, delayTicks: [5, 20] }, { templateIndex: 2, probability: 0.7, delayTicks: [10, 30] }, { templateIndex: 19, probability: 0.5, delayTicks: [30, 60] }], lessonText: '교훈: 예측할 수 없는 블랙스완 이벤트에 대비하세요. 항상 최악의 시나리오를 염두에 두고 분산 투자하세요.' },
  ...
  // ═══ 2002-2003 ═══
  { year: 2002, month: 1, title: '월드컴 회계부정', description: '월드컴 대규모 회계부정 사건으로 기업 신뢰도가 추락합니다.', type: 'company', impact: { driftModifier: -0.10, volatilityModifier: 0.25, severity: 'high' }, duration: 100, weight: 2, affectedSectors: ['telecom', 'finance'] },
  ...
  { year: 2002, month: 9, title: '카드 대란 시작', description: '신용카드 연체율이 급증하며 금융 위기 우려가 커집니다.', type: 'crash', impact: { driftModifier: -0.12, volatilityModifier: 0.25, severity: 'high' }, duration: 150, weight: 2, affectedSectors: ['finance', 'consumer'] },
  ...
  { year: 2006, month: 5, title: '글로벌 증시 조정', description: '신흥국 과열 우려로 글로벌 증시가 동시 조정됩니다.', type: 'crash', impact: { driftModifier: -0.10, volatilityModifier: 0.25, severity: 'high' }, duration: 60, weight: 2 },
  ...
  { year: 2007, month: 7, title: '베어스턴스 헤지펀드 파산', description: '서브프라임 관련 헤지펀드 2개가 파산합니다.', type: 'crash', impact: { driftModifier: -0.10, volatilityModifier: 0.25, severity: 'high' }, duration: 80, weight: 2, affectedSectors: ['finance'] },
  ...
  { year: 2008, month: 3, title: '베어스턴스 인수 합병', description: 'JP모건이 베어스턴스를 헐값에 인수하며 공포가 확산됩니다.', type: 'crash', impact: { driftModifier: -0.12, volatilityModifier: 0.3, severity: 'high' }, duration: 80, weight: 2, affectedSectors: ['finance'] },
  ...
  // v7.5: 2008 금융위기 drift 반감
  { year: 2008, month: 9, title: '리먼 브라더스 파산', description: '리먼 브라더스가 파산하며 글로벌 금융 시스템이 마비됩니다.', type: 'crash', impact: { driftModifier: -0.12, volatilityModifier: 0.5, severity: 'critical' }, duration: 300, weight: 1, chainEvents: [{ templateIndex: 0, probability: 0.95, delayTicks: [5, 15] }, { templateIndex: 3, probability: 0.8, delayTicks: [20, 50] }, { templateIndex: 8, probability: 0.9, delayTicks: [30, 80] }, { templateIndex: 9, probability: 0.7, delayTicks: [40, 100] }], lessonText: '교훈: 시스템 위기 시 분산투자는 무력합니다. 모든 자산이 동시에 하락할 때, 현금만이 방패입니다.' },
  { year: 2008, month: 10, title: '글로벌 주식시장 대폭락', description: '전세계 증시가 동반 폭락, 코스피 40% 이상 하락.', type: 'crash', impact: { driftModifier: -0.11, volatilityModifier: 0.4, severity: 'critical' }, duration: 80, weight: 1 },
  ...
  // ═══ 2009-2011 ═══
  { year: 2009, month: 3, title: '미 연준 양적완화 시작', description: '대규모 양적완화로 유동성이 풍부해집니다.', type: 'policy', impact: { driftModifier: 0.08, volatilityModifier: 0.15, severity: 'high' }, duration: 200, weight: 2, chainEvents: [{ templateIndex: 7, probability: 0.7, delayTicks: [50, 100] }], lessonText: '교훈: 중앙은행이 돈을 풀면 자산 가격은 오릅니다. 통화정책의 방향을 읽는 것이 투자의 핵심입니다.' },
  ...
  { year: 2010, month: 4, title: '유럽 재정위기 (그리스)', description: '그리스 국가 부도 위기로 유럽 전체가 흔들립니다.', type: 'global', impact: { driftModifier: -0.12, volatilityModifier: 0.3, severity: 'high' }, duration: 150, weight: 2, affectedSectors: ['finance'] },
  { year: 2010, month: 5, title: '플래시 크래시', description: '다우지수가 몇 분 만에 1000포인트 폭락합니다.', type: 'crash', impact: { driftModifier: -0.12, volatilityModifier: 0.5, severity: 'high' }, duration: 20, weight: 2 },
  ...
  { year: 2011, month: 8, title: '미국 신용등급 강등', description: 'S&P가 미국 신용등급을 AAA에서 강등합니다.', type: 'global', impact: { driftModifier: -0.12, volatilityModifier: 0.25, severity: 'high' }, duration: 80, weight: 2, affectedSectors: ['finance'] },
  ...
  { year: 2013, month: 5, title: '테이퍼 탠트럼', description: '양적완화 축소 시사에 신흥국 시장이 급락합니다.', type: 'crash', impact: { driftModifier: -0.10, volatilityModifier: 0.25, severity: 'high' }, duration: 80, weight: 2, chainEvents: [{ templateIndex: 4, probability: 0.6, delayTicks: [20, 50] }, { templateIndex: 13, probability: 0.5, delayTicks: [10, 30] }] },
  ...
  // ═══ 2015-2016 ═══
  { year: 2015, month: 6, title: '중국 증시 대폭락', description: '중국 상하이 종합지수가 40% 이상 폭락합니다.', type: 'crash', impact: { driftModifier: -0.18, volatilityModifier: 0.4, severity: 'critical' }, duration: 120, weight: 1, chainEvents: [{ templateIndex: 4, probability: 0.7, delayTicks: [20, 50] }, { templateIndex: 13, probability: 0.6, delayTicks: [10, 30] }], lessonText: '교훈: 정부가 시장을 통제하려 할수록 변동성은 커집니다. 인위적 상승장을 맹신하지 마세요.' },
  { year: 2015, month: 8, title: '위안화 절하 충격', description: '중국이 위안화를 갑자기 절하하며 글로벌 패닉.', type: 'global', impact: { driftModifier: -0.12, volatilityModifier: 0.3, severity: 'high' }, duration: 80, weight: 2 },
  ...
  { year: 2016, month: 6, title: '브렉시트 국민투표', description: '영국의 EU 탈퇴 투표 결과에 글로벌 시장이 충격.', type: 'global', impact: { driftModifier: -0.12, volatilityModifier: 0.35, severity: 'high' }, duration: 60, weight: 2, chainEvents: [{ templateIndex: 1, probability: 0.6, delayTicks: [20, 40] }] },
  ...
  { year: 2018, month: 2, title: '변동성 쇼크 (VIX 폭등)', description: 'VIX가 하루 만에 3배 급등하며 글로벌 매도세.', type: 'crash', impact: { driftModifier: -0.12, volatilityModifier: 0.4, severity: 'high' }, duration: 40, weight: 2 },
  ...
  { year: 2018, month: 10, title: '기술주 대폭락', description: 'FAANG 중심 기술주가 일제히 급락합니다.', type: 'crash', impact: { driftModifier: -0.15, volatilityModifier: 0.35, severity: 'high' }, duration: 80, weight: 2, affectedSectors: ['tech'] },
  { year: 2018, month: 12, title: '크리스마스 이브 폭락', description: '미 증시가 크리스마스 이브에 대폭락합니다.', type: 'crash', impact: { driftModifier: -0.12, volatilityModifier: 0.3, severity: 'high' }, duration: 30, weight: 2, chainEvents: [{ templateIndex: 1, probability: 0.7, delayTicks: [10, 20] }] },
  ...
  // ═══ 2022 ═══
  { year: 2022, month: 1, title: '긴축 공포', description: '연준의 공격적 긴축 시사로 성장주가 급락합니다.', type: 'policy', impact: { driftModifier: -0.12, volatilityModifier: 0.25, severity: 'high' }, duration: 120, weight: 2, affectedSectors: ['tech'] },
  { year: 2022, month: 2, title: '러시아-우크라이나 전쟁', description: '러시아가 우크라이나를 침공하며 글로벌 충격.', type: 'global', impact: { driftModifier: -0.18, volatilityModifier: 0.4, severity: 'critical' }, duration: 200, weight: 1, affectedSectors: ['energy', 'materials'], chainEvents: [{ templateIndex: 16, probability: 0.9, delayTicks: [10, 30] }, { templateIndex: 15, probability: 0.6, delayTicks: [30, 60] }], lessonText: '교훈: 지정학 리스크는 에너지·원자재 가격을 급등시킵니다. 전쟁 시에는 방산·에너지 섹터가 수혜를 받습니다.' },
  ...
  { year: 2024, month: 8, title: '일본 엔 캐리 청산 쇼크', description: '엔화 급등으로 글로벌 캐리 트레이드가 대거 청산됩니다.', type: 'crash', impact: { driftModifier: -0.15, volatilityModifier: 0.4, severity: 'high' }, duration: 40, weight: 2 },
