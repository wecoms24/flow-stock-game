...
- `chapterProgress: ChapterProgress` — Chapter/campaign objective tracking
- `prestigeData: PrestigeData` — New Game+ progression (level, carry-over skill, cash bonus)
- `celebrationState: CelebrationState` — Unified celebration event queue
- `isFastForwarding: boolean` — Time skip mode active flag
- `fastForwardProgress: FastForwardProgress` — Time skip progress (current, skippedHours, events, startTime)
...
19. Chapter objective evaluation — monthly check against chapter goals
20. Employee milestone checks — on level-up, tenure change, performance events
21. Celebration dispatch — queued celebrations processed via CelebrationManager
...
| `chapterEngine.ts` | Chapter-based campaign progression, objective evaluation, feature unlocks | Monthly |
| `employeeMilestoneEngine.ts` | Employee growth milestone tracking, condition checks, reward grants | On level-up + events |
| `endgameRecapEngine.ts` | 30-year retrospective data aggregation (star employees, timeline, competitors) | Game end |
| `decisionAnalysisEngine.ts` | Trading pattern analysis (sector breakdown, timing, risk behavior) | Game end |
...
| `kisConfig.ts` | 한국투자증권 OpenAPI WebSocket URLs, TR IDs, reconnect backoff |
| `hybridModeConfig.ts` | Real-time KOSPI hybrid mode correction strength (crisis/normal) |
...
- `chapters.ts`: 6+ chapter definitions with year ranges, objectives, and unlocked features per chapter
- `employeeMilestones.ts`: 30+ employee milestone definitions (growth/tenure/performance/contribution)
- `employeeTestimonials.ts`: Role-based and emotion-based employee testimonial templates for endgame recap
- `employeeEmoji.ts`: Character emoji system (role emoji, behavior emoji, mood faces)
- `metaAchievements.ts`: Meta-game achievement definitions + NG+ bonus definitions
- `kospiCompanies.ts`: 100 real KOSPI company data (auto-generated by `scripts/build_kospi_db.py`)
...
- `prestigeSystem.ts`: New Game+ system (level 0-10, carry-over corporate skill, cash multiplier)
- `celebrationSystem.ts`: Unified 3-level celebration events (Toast/Banner/Ceremony with confetti/sound)
- `metaProgressionSystem.ts`: Cross-playthrough achievement tracking + NG+ bonus unlocks
- `sqlite/`: SQLite WASM persistence subsystem (`database.ts`, `queries.ts`, `transformers.ts`, `migration.ts`, schema v10)
- `personalization/profile.ts`: Behavioral profiling (risk tolerance, play pace, attention, learning stage)
...
**Window Types** (22): `portfolio`, `chart`, `trading`, `news`, `office`, `office_dot`, `office_history`, `employee_detail`, `ranking`, `settings`, `ending`, `institutional`, `proposals`, `acquisition`, `dashboard`, `achievement_log`, `monthly_cards`, `event_chain_tracker`, `skill_library`, `training_center`, `playstyle_analytics`, `isometric_office`
...
### Chapter / Campaign System (`src/engines/chapterEngine.ts`, `src/data/chapters.ts`)

The 30-year game (1995-2025) is divided into themed chapters, each with objectives and feature unlocks.

**6 Chapters** (each ~5 years):
1. **창업기** (1995-1999): Survival basics, hire first employees
2. **닷컴 시대** (2000-2004): Achieve profitability, grow net worth
3. **성장기** (2005-2009): Team expansion, crisis recovery
4. **회복기** (2010-2014): Rank climbing, office expansion
5. **골드러시** (2015-2019): Tech boom, wealth accumulation
6. **패권** (2020-2025): Final victory conditions

**Chapter Progress** (`ChapterProgress`): Tracked in store, evaluated monthly against chapter-specific objectives (e.g., cash thresholds, employee counts, rank targets). Chapter transitions trigger `ChapterModal` UI.

**Types**: `src/types/chapter.ts` — `Chapter`, `ChapterObjective`, `ChapterProgress`, `InvestmentStyle`

### Prestige / New Game+ System (`src/systems/prestigeSystem.ts`, `src/types/prestige.ts`)

Meta-progression system enabling replayability across playthroughs.

- **Prestige Levels**: 0-10 (max)
- **Cash Multiplier**: +5% per level (up to 50% at level 10)
- **Carry-Over**: 1 corporate skill persists to next run
- **Tracking**: Best final assets, best ROI across all runs
- **Storage**: `localStorage('retro-stock-prestige')` — separate from game saves
- **Integration**: Prestige bonuses applied at game start via `PrestigeBonuses`

### Meta Achievement System (`src/systems/metaProgressionSystem.ts`, `src/data/metaAchievements.ts`)

Cross-playthrough achievements that persist across game resets and NG+ runs.

- Achievements: `billionaire_club`, `crisis_survivor`, `best_employer`, `sector_king`, `diverse_portfolio`, etc.
- Each achievement unlocks NG+ bonuses
- Tracks: best ROI, total games played, cumulative stats

### Celebration System (`src/systems/celebrationSystem.ts`, `src/types/celebration.ts`)

Unified 3-level celebration event system dispatched via `dispatchCelebration()`.

| Level | Type | Duration | Position | Effects |
|-------|------|----------|----------|---------|
| 1 | Toast | 4s | Bottom-right | Small notification |
| 2 | Banner | 6s | Full-width top | Sound effect |
| 3 | Ceremony | Manual dismiss | Center modal | Confetti + fanfare |

**UI Components**: `CelebrationManager.tsx` (queue handler), `CelebrationToast.tsx`, `CeremonyOverlay.tsx`

**Triggers**: Rank overtakes, skill unlocks, employee milestones, chapter completions, achievements

### Employee Milestone System (`src/engines/employeeMilestoneEngine.ts`, `src/data/employeeMilestones.ts`)

Individual employee achievement tracking with 30+ milestone definitions across 4 categories:
- **Growth**: Level-up thresholds, skill mastery
- **Tenure**: Longevity milestones (1yr, 5yr, etc.)
- **Performance**: Trade success streaks, proposal accuracy
- **Contribution**: Portfolio impact, team synergy

Milestones grant rewards (satisfaction boost, stress reduction, XP bonus). Tracked in `employeeBio`, aggregated in endgame recap.

### Endgame Recap System (`src/engines/endgameRecapEngine.ts`, `src/engines/decisionAnalysisEngine.ts`)

Generates comprehensive 30-year retrospective at game conclusion.

**EndgameRecap** (`src/types/endgame.ts`):
- `summary`: Final assets, ROI, ranking, game duration
- `timeline`: Asset curve over 30 years with turning points
- `starEmployees`: Best performers with testimonials (`src/data/employeeTestimonials.ts`) and milestone counts
- `competitorResults`: Final rankings with head-to-head win/loss records
- `decisionAnalysis`: Sector breakdown, timing patterns, risk behavior assessment
- `turningPoints`: Key milestones (first billion, first master employee, rank 1, etc.)
- `narrative`: Generated story summary of the player's journey

### Fast Forward / Time Skip System

Allows players to skip forward through game hours with event collection.

- **Store Actions**: `fastForward()`, `cancelFastForward()`
- **UI**: `FastForwardOverlay.tsx` — progress bar with skipped hours and collected events
- **Behavior**: Processes ticks at maximum speed, collects notable events, shows summary on completion

### Real-Time Market Mode (`src/config/kisConfig.ts`, `src/types/realtime.ts`)

Integration with 한국투자증권 (Korea Investment Securities) OpenAPI for real-time KOSPI data.

- **GameMode**: `'virtual'` (default simulation) | `'kospi'` (historical KOSPI) | `'realtime'` (live KIS data)
- **WebSocket**: Real-time price streaming (TR ID: `H0STCNT0`)
- **Hybrid Mode** (`hybridModeConfig.ts`): Corrects GBM simulation prices toward real KOSPI closes (crisis correction 0.8, normal 0.15)
- **KOSPI Companies** (`src/data/kospiCompanies.ts`): 100 real companies with historical parameters, auto-generated by `scripts/build_kospi_db.py`
- **Market Closed Dialog**: `MarketClosedDialog.tsx` — shows when real market is closed, offers switch to virtual mode

### Component Architecture

**Component Directories**:
- `src/components/desktop/` (9 files): Taskbar, StartScreen, MainDashboard, StockTicker, NewsFeed, ObjectiveWidget, CompetitorWidget, EmployeeCardGrid, QuickTradePanel
- `src/components/windows/` (25+ files): All window type implementations + WindowFrame + WindowManager
- `src/components/ui/` (21+ files): Reusable UI components (RetroButton, RetroPanel, BadgeIcon, XPBar, ProgressGauge, NotificationCenter, etc.)
- `src/components/effects/` (9 files): Visual effects (CRTOverlay, FloatingText, StockParticles, TradeAnimationSequence, NumberCounter)
- `src/components/office/` (4 files): Office rendering (OfficeCanvas, SpeechBubble, SynergyLines, BlueprintOverlay)
- `src/components/tutorial/` (3 files): ChapterModal, OfficeTutorial, Week1Demo
...
25. **Prestige data isolation**: Prestige/NG+ data lives in `localStorage('retro-stock-prestige')`, NOT in game saves. Never mix prestige state into SQLite/IndexedDB save data
26. **Celebration queue ordering**: `dispatchCelebration()` queues events; `CelebrationManager` processes them in order. Don't show celebrations directly — always go through the queue
27. **Chapter objective evaluation**: Chapter checks run monthly — don't evaluate objectives every tick. Chapter transitions must show `ChapterModal` before unlocking new features
28. **Fast forward cleanup**: After `fastForward()` completes, must reset `isFastForwarding` and process collected events summary. Cancellation via `cancelFastForward()` must be idempotent
29. **GameMode type**: `GameMode` is `'virtual' | 'kospi' | 'realtime'` — UI components must handle all three modes, especially trading windows and price displays
30. **Employee milestone rewards**: Milestone rewards (satisfaction/XP) are applied once on unlock — guard against duplicate grants by checking `employee.milestones` array before adding
31. **Endgame recap data**: `endgameRecapEngine` aggregates from multiple store slices — ensure all data (cashFlowLog, realizedTrades, competitors, employees) is available before generating recap
32. **SQLite schema version**: Currently at v10 — supports speed 8x/16x and tradeStreak persistence. Always increment schema version when modifying save format
...
- **Fast forward batching**: Time skip processes ticks at max speed without rendering intermediate states — only final state is rendered
- **Celebration deduplication**: CelebrationManager prevents duplicate celebrations for the same event within a short window
- **Endgame recap**: Heavy computation (decision analysis, timeline aggregation) runs once at game end — results are cached, not recalculated on re-render
